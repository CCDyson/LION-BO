...
# and instead of using the transmission to calculate the cost, it needs to be chi squared

def cost_function(params, nEvts):

    # Collects drifts and calculates third drift
    ...
    iEO.setBLEparams([
        [startBLEparams[0][0], drift1],
        [startBLEparams[1][0], 0, 0.01],
        [startBLEparams[2][0],  0.04, 150.],
        [startBLEparams[3][0], 0, 0.01],
        [startBLEparams[4][0], drift2],
        [startBLEparams[5][0], 0, 0.01],
        [startBLEparams[6][0], 0.02, 150.],
        [startBLEparams[7][0], 0, 0.01],
        [startBLEparams[8][0], drift3]
    ])
    ...
    # This is the function that needs to be modified to calculate the chi squared
    # The function itself is in the EnvelopeBayesianOptimisation.py file
    transmission = iEO.calculate_transmission()
    cost = 100 - transmission
...
# This is the main part of the script
    ...
    experiment_files = '1RCF3.1.csv', '1RCF6.1.csv', '1RCF8.2.csv', '1RCF9.9.csv', '1RCF11.4', '1RCF12.7', '1RCF13.9.csv']
    df_list = [pd.read_csv(f) for f in experiment_files]
    exp_data = pd.concat(df_list)
    
    for stack in stack_fn_cut:
        with open(f"stack_response_functions/stack_{stack}_response_function.pkl", "rb") as file:
    ...
    res = gp_minimize(lambda x: cost_function(x, nEvts), \
                      space, \
                      n_calls=150, \
                      n_initial_points=0, \
                      random_state=None, \
    ...
    with open("99-Scratch/optimisation_results.csv", "w", newline="") as f:
        writer = csv.writer(f)
        writer.writerow(["x1", "y1", "a1", "b1", "g1", "x2", "y2", "a2", "b2", "g2", "chi2"])  
    ...
    for idx, state in enumerate(random_states):
        ...
        print('************************************')
        res = gp_minimize(lambda x: cost_function(x, nEvts), space, \
                      n_calls=150, \
                      n_initial_points=20, \
                      random_state=state, \
                      acq_func="EI", \
    ...
    results_df = pd.DataFrame(results, columns=["x1", "y1", "a1", "b1", "g1", "x2", "y2", "a2", "b2", "g2", "chi2"])
    
    averaged_data = results_df.groupby(["x1", "y1", "a1", "b1", "g1", "x2", "y2", "a2", "b2", "g2"])['chi2'].mean().reset_index()
    
    best_params = averaged_data.loc[averaged_data['chi2'].idxmax()]


    # Extract optimal parameters
    optimal_drift1 = best_params['d1']/1000
    optimal_drift2 = best_params['d2']/1000
    optimal_total_length = best_params['total length']/1000
    optimal_drift3 = optimal_total_length - optimal_drift1 - optimal_drift2 - 0.04 - 0.02
    best_trans = best_params['transmission']
...
#---- Runs the simulation with the optimal parameters:
    ...
    startBLEparams = iEO.getstartBLEparams()
    iEO.setBLEparams([
        [startBLEparams[0][0], optimal_drift1],
        [startBLEparams[1][0], 0, 0.01],
        [startBLEparams[2][0],  0.04, 150.],
        [startBLEparams[3][0], 0, 0.01],
        [startBLEparams[4][0], optimal_drift2],
        [startBLEparams[5][0], 0, 0.01],
        [startBLEparams[6][0], 0.02, 150.],
        [startBLEparams[7][0], 0, 0.01],
        [startBLEparams[8][0], optimal_drift3]
    ])
